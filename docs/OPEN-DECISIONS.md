...
> The list of things not yet decided, each with the current **recommended default**. Any doc or design that meets a TBD uses the default and says so. When a decision is made, record it here (date, choice), then propagate to the docs. Last updated 2026-09-05 (decisions 2, 7, 12, 13 resolved; 21 added).
...
| 21 | **In-app account creation** for sponsors with no nostr identity | (a) none: sponsors must arrive with a key (extension or remote signer); (b) reuse brainstorm.world's flow: generate the key in the browser, hold it encrypted at rest (device-key wrap in IndexedDB + localStorage), prompt for a password-encrypted NIP-49 backup file and a password-manager entry, never send the key to a server (reviewed 2026-09-05 in Brainstorm-UI: `client/src/services/nostr.ts`, `lib/skVault.ts`, `lib/accountBackup.ts`, `lib/credentialManager.ts`) | (b), sharing the code with Brainstorm-UI rather than re-implementing; note that a brand-new key has no web of trust and will fail the trust check until vouched, so the join flow must say so before creating the account | open |
